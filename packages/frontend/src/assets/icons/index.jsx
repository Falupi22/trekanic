import React from "react";
import { ReactComponent as _FrameLogo } from "./frame.svg";
import { ReactComponent as _FlatTireLogo } from "./flat-tire.svg";
import { ReactComponent as _BrakeLogo } from "./brake.svg";
import { ReactComponent as _EllipsisLogo } from "./ellipsis.svg";
import { ReactComponent as _BinLogo } from "./bin.svg";
import { ReactComponent as _PencilLogo } from "./pencil.svg";
import { ReactComponent as _PlusLogo } from "./plus.svg";
import { ReactComponent as _CloseLogo } from "./close.svg";
import { ReactComponent as _CheckLogo } from "./check.svg";
import { ReactComponent as _LogOutLogo } from "./logout.svg";
import customIcons from "./customIcons"

export const FrameLogo = function () {
    return <_FrameLogo />
}

export const FlatTireLogo = function () {
    return <_FlatTireLogo />
}

export const BrakeLogo = function () {
    return <_BrakeLogo />
}

export const EllipsisLogo = function () {
    return <_EllipsisLogo />
}

export const BinLogo = function () {
    return <_BinLogo />
}

export const PencilLogo = function () {
    return <_PencilLogo />
}

export const PlusLogo = function () {
    return <_PlusLogo />
}

export const CloseLogo = function () {
    return <_CloseLogo />
}

export const CheckLogo = function () {
    return <_CheckLogo />
}

export const LogOutIcon = customIcons.logOutIcon