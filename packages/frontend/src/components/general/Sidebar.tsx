...
export default function Sidebar({ username }) {
    ...
    return (
        <Box minH="100vh" bg={'dark.500'}
            display="flex"
        >
            <SidebarContent username={username} logOutCallback={logout} onClose={() => onClose} display={{ base: 'none', md: 'block' }} />
            {/* mobilenav */}
...
const SidebarContent = ({ onClose, logOutCallback, username, ...rest }: SidebarProps) => {
    return (
        <Box mt={20}
            bg={'dark.500'}
            borderRight="1px"
                ...
                >
                    <Flex
                        ...
                        <HStack align="center"
                            ...
                            role="group"
                            cursor="pointer"
                            color='white' mt="370" _hover={{
                                bg: 'dark.400',
                                color: 'white',
                            }}>
                            <LogOutIcon />
                            <Text ml={2} color="while">Log out</Text>
                        </HStack>
                    </Flex>
                </Box>
            </Box>
